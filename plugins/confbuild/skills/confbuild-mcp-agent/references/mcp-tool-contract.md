...
| Export | `confbuild_export_project` | Starts an export through the app’s own exporters in server-side Chromium with browser-tab fallback: `step` (simple/complex, exact OCCT B-reps where available), `stl`, `glb`, `3mf`, sanitized `bom-json`/`bom-csv`, `drawing-pdf`, `sheetmetal-dxf`, `cost-json`, `quote-pdf`, `purchase-json`/`purchase-csv`, or `variant-metrics-json`. Complex STEP can take minutes on large models |
